...
sampdata = read_csv("normpeno_pos.csv") %>% remove_rownames() %>% column_to_rownames(var = "sample") %>% as.data.frame()

# Import feature data
fdata <- read_csv("normpro_pos.csv") %>% remove_rownames() %>% column_to_rownames(var = "X1") %>% as.data.frame()
...
for (i in 3:1853) {
    qcrlsc_pos[, i] <- qcrlsc_pos[, i]/qcrlsc_pos[, "rowsum"]
}
...
for (i in 1:16) {
    qc_pos_acm[i + 32, 3:1853] <- qc_pos_ac[i, 3:1853] - qc_pos_ac[i + 16, 3:1853]
}
...
# 1.0 mins using 7 threads on a Mac Powerbook Pro mid-2015 with 2,8 GHz Intel Core i7.  Stop parallel processing
stopCluster(cl)
