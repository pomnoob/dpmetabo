...
normABC_pos <- norm_pos[norm_pos$class == "a" | norm_pos$class == "b" | norm_pos$class == "c", ] %>% dplyr::arrange(sample) %>% dplyr::slice(-43, 
    -44)
...
plotIndiv(plsda.ad, ind.names = ml, group = group, legend = TRUE, title = "AD multilevel PLS-DA, comp 1 - 2")

background = background.predict(plsda.ad, comp.predicted = 2, dist = "max.dist")

plotIndiv(plsda.ad, comp = 1:2, group = group, ind.names = FALSE, title = "Maximum distance", legend = TRUE, background = background)

set.seed(2543)  # for reproducibility, only when the `cpus' argument is not used
perf.plsda.ad <- perf(plsda.ad, validation = "Mfold", folds = 5, progressBar = FALSE, auc = TRUE, nrepeat = 10)
...
tune.splsda.ad <- tune.splsda(tsAD_pos_m, Y = group, multilevel = ml, ncomp = 2, validation = "Mfold", folds = 5, progressBar = TRUE, dist = "max.dist", 
    measure = "BER", test.keepX = list.keepX, nrepeat = 10, cpus = 2)

tune.splsda.ad7 <- tune.splsda(tsAD_pos_m, Y = group, multilevel = ml, ncomp = 7, validation = "Mfold", folds = 5, progressBar = TRUE, dist = "max.dist", 
    measure = "BER", test.keepX = list.keepX, nrepeat = 10, cpus = 2)
...
plotIndiv(splsda.ad, group = group, ind.names = FALSE, ellipse = TRUE, legend = TRUE, title = "sPLS-DA on SRBCT, comp 1 & 2")
...
plotIndiv(plsda.abc, ind.names = ml3, group = group3, legend = TRUE, title = "ABC multilevel PLS-DA, comp 1 - 2")

background = background.predict(plsda.abc, comp.predicted = 2, dist = "max.dist")

plotIndiv(plsda.abc, comp = 1:2, group = group3, ind.names = FALSE, title = "Maximum distance", legend = TRUE, background = background)

set.seed(2543)  # for reproducibility, only when the `cpus' argument is not used
perf.plsda.abc <- perf(plsda.abc, validation = "Mfold", folds = 5, progressBar = FALSE, auc = TRUE, nrepeat = 10)
...
tune.splsda.abc <- tune.splsda(tsABC_pos_m, Y = group3, multilevel = ml3, ncomp = 3, validation = "Mfold", folds = 5, progressBar = TRUE, dist = "max.dist", 
    measure = "BER", test.keepX = list.keepX, nrepeat = 10, cpus = 2)
...
plotIndiv(splsda.abc, group = group3, ind.names = FALSE, ellipse = TRUE, legend = TRUE, title = "sPLS-DA on ABC, comp 1 & 2")
...
perf.abc <- perf(splsda.abc, validation = "Mfold", folds = 5, dist = "max.dist", nrepeat = 10, progressBar = FALSE)
...
par(mfrow = c(1, 2))
plot(perf.abc$features$stable[[1]], type = "h", ylab = "Stability", xlab = "Features", main = "Comp 1", las = 2)
plot(perf.abc$features$stable[[2]], type = "h", ylab = "Stability", xlab = "Features", main = "Comp 2", las = 2)

par(mfrow = c(1, 1))

# here we match the selected variables to the stable features
ind.match = match(selectVar(splsda.abc, comp = 1)$name, names(perf.abc$features$stable[[1]]))
